...
const getGeojson = (regionCode: string) => {
  return new Promise<boolean>(async (resolve) => {
    ...
    console.log("get geo map", regionCode, mapjson)
    if (mapjson) {
      mapjson = mapjson.geoJSON;
      resolve(mapjson);
    console.log("1 get geo map", regionCode, mapjson)
    } else {
      mapjson = await GETNOBASE(`./map-geojson/${regionCode}.json`).then((data) => data);
    console.log("2 get geo map", `./map-geojson/${regionCode}.json`, regionCode, mapjson)
